...
from drvl import DRVL, utcnow_iso
...
def create_signed_event(event_data: dict) -> dict:
    """Attach policy hash and signature deterministically"""
    ...
    # verification uses same deterministic payload
...
def run_demo():
    ...
    envelope = drvl.ExecutionEnvelope(action, table)
    ...
    return jsonify({
        ...
        "escalation_queue": [{"request_id": q["id"], "action": q["action"], "table": q["table"], "status": q["status"], "envelope_hash": q["envelope_hash"]} for q in escalation_queue],
